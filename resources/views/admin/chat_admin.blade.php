...
<div class="min-h-screen bg-gradient-to-br from-blue-50 to-sky-100 flex flex-col items-center py-12">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-5xl p-6">
        <h2 class="text-2xl font-bold text-blue-700 text-center mb-6"> Panel de Chat de Apoyo - Administrador</h2>
...
function renderMessages(messages) {
    ...
    messages.forEach(m => {
        ...
        bubble.className = `p-3 rounded-xl max-w-xs ${m.usuario === 'Administrador' ? 'bg-blue-100 self-end ml-auto' : 'bg-gray-200 self-start'}`;
        bubble.innerHTML = `<strong>${m.usuario}</strong><br>${m.mensaje}<br><small>Riesgo: ${m.severity ?? 'No analizado'}</small>`;
...
async function fetchMessages() {
    const res = await fetch("{{ route('admin.chat_admin') }}", { headers: {'X-Requested-With':'XMLHttpRequest'} });
    if (res.ok) {
